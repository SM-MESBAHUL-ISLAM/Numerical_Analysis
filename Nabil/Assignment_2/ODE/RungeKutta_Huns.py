...
def RungeKutta_HunsMethod():
    xi,yi,xn = (map(float,input("Please Enter x(initial),y(initial) and x(final) : ").split(",")))
    total_no_of_stepSize = int(input("Enter the number of step size: "))
    h = []
    print("Enter the step sizes: ")
    for i in range(0,total_no_of_stepSize):
    ...
    def HeunsMethod(xi, yi, xn, h, total_stepsize):
        drawCurve(xi, xn)
        ...
        for i in range(0, total_stepsize):
            ...
            for j in range(0, steps_need):
                ...
                k1 = dydx(cur_x, cur_y)
                k2 = dydx(cur_x + h[i], cur_y + (k1 * h[i]))
                cur_y = cur_y + ((k1 + k2) * h[i] * 0.5)
                cur_x = h[i] + cur_x
            ...
            print("Answer using step size h= " + f'{h[i]:.3f}' + " =" + f'{cur_y:.3f}' + "\n")
        ...
        plt.legend()
        plt.show()
    HeunsMethod(xi,yi,xn,h,total_no_of_stepSize)