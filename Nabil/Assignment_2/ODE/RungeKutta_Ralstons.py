...
def RungeKutta_RalstonsMethod():
    xi,yi,xn = (map(float,input("Please Enter x(initial),y(initial) and x(final) : ").split(",")))
    total_number_of_stepSizes = int(input("Enter the number of step size: "))
    h = []
    print("Enter the step sizes: ")
    for i in range(0,total_number_of_stepSizes):
    ...
    def RalstonMethod(xi, yi, xn, h, total_stepsize):
        drawCurve(xi, xn)
        ...
        for i in range(0, total_stepsize):
            ...
            for j in range(0, steps_need):
                ...
                k1 = dydx(cur_x, cur_y)
                k2 = dydx(cur_x + (h[i] * 0.75), cur_y + (k1 * h[i] * 0.75))
                cur_y = cur_y + ((k1 * h[i]) / 3) + ((k2 * h[i] * 2) / 3)
                cur_x = h[i] + cur_x
    ...
    RalstonMethod(xi,yi,xn,h,total_number_of_stepSizes)